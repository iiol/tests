...
void
setcolor1(uint32_t *buf, uint32_t color, int h, int w)
{
	int i, j;
...
void
setcolor2(uint32_t *buf, uint32_t color, int h, int w)
{
	int i, j;
...
int main()
{
	...
	/* draw something */
#if 1
	dev->width = dev->width + 10;
	printf("w: %d, h: %d\n", dev->width, dev->height);
	while (1) {
		for (r = 0xFF; r != 0x00; r--, g++) {
			//setcolor(dev->buf, (r << 16) + (g << 8), dev->height, dev->width);
			setcolor2(dev->buf, (g << 16) + (r << 8), dev->height, dev->width);
			usleep(1);
		}

		for (g = 0xFF; g != 0x00; g--, b++) {
			//setcolor(dev->buf, (g << 8) + b, dev->height, dev->width);
			setcolor2(dev->buf, (b << 8) + g, dev->height, dev->width);
			usleep(1);
		}

		for (b = 0xFF; b != 0x00; b--, r++) {
			//setcolor(dev->buf, (r << 16) + b, dev->height, dev->width);
			setcolor2(dev->buf, (b << 16) + r, dev->height, dev->width);
			usleep(1);
		}
	}
#else
	while (1) {
		setcolor(dev->buf, 0xFFFFFF, dev->height, dev->width);
		sleep(1);
		setcolor(dev->buf, 0x000000, dev->height, dev->width);
		sleep(1);
	}
#endif
